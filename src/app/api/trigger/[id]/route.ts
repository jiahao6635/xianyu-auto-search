  ...
  try {
    ...
    if (!hasCookie && !hasLocalProfile) {
      return NextResponse.json({
        success: false,
        error: '未配置登录态，请填写 Cookie，或配置本地已登录浏览器用户目录',
        hint: '可选方案：1. 在浏览器登录闲鱼后，从 Network 请求复制 Cookie；2. 配置 browser_user_data_dir 指向本机已登录浏览器的用户目录',
      });
    }
    ...
    const result = await enqueueMonitorTask(`manual#${config.id}:${config.search_keyword}`, async () => {
      ...
      try {
        ...
        if (newProducts.length > 0 && config.webhook_url) {
          ...
          console.log('[trigger] 没有新商品，跳过 webhook 和 sent_products');
        }
